...
function setupAudioListeners() {
  window.addEventListener('audio:statechange', (e) => {
    ...
    if (playBtn) {
      playBtn.className = e.detail.isPlaying ? 'bi bi-pause-fill' : 'bi bi-play-fill';
    }
...
function setupTheme() {
  ...
  if (themeBtn) {
    themeBtn.innerHTML = saved === 'dark' ? '<i class="bi bi-sun"></i>' : '<i class="bi bi-moon"></i>';
    themeBtn.onclick = () => {
...
async function renderHome(container) {
  container.innerHTML = `
    <div class="card p-5 text-center" style="margin-bottom:20px;">
      ...
      <p class="mt-2 text-muted">Loading your Hifz Dashboard...</p>
    </div>
  `;
  ...
  try {
    ...
    container.innerHTML = `
      ...
      <div class="hero-card" style="margin-bottom: 24px;">
        <div>
          <div class="hero-greeting">Assalamu Alaikum, ${user ? (user.name || user.username) : 'Learner'}</div>
          <div class="hero-sub">May Allah bless your dedication to preserving His noble verses in your heart.</div>
          <div class="hero-stats-row">
            <div class="hero-stat-box">
              <div class="hero-stat-val">${streak} Days</div>
      ...
      <div class="dashboard-grid">
          ...
          <div class="card card-primary-edge">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
              <h3 style="font-size:1.1rem; font-weight:700; color:var(--primary);">
                <i class="bi bi-book-half"></i> Today's Hifz Target
              </h3>
            ...
            <div>
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                <h3 style="font-size:1.1rem; font-weight:700; color:var(--gold);">
                  <i class="bi bi-clock-history"></i> Today's Muraja'ah
                </h3>
              ...
              ${weakCount > 0 ? `
                <div style="background:#fee2e2; border:1px solid #fca5a5; border-radius:var(--radius-sm); padding:10px 14px; margin-top:14px; display:flex; align-items:center; justify-content:space-between;">
                  <div style="display:flex; align-items:center; gap:8px; color:#991b1b; font-weight:600; font-size:0.85rem;">
                    <i class="bi bi-exclamation-triangle-fill"></i> ${weakCount} Weak Verses Found
                  </div>
              ...
              ` : `
                <div style="background:var(--primary-subtle); border-radius:var(--radius-sm); padding:10px 14px; margin-top:14px; color:var(--primary); font-size:0.85rem; font-weight:600;">
                  <i class="bi bi-check-circle-fill"></i> All reviewed verses are in solid shape!
                </div>
              `}
            </div>
        ...
        <!-- Quick Access / Analytics Preview -->
        <div class="col-12">
          <div class="card">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
              <h3 style="font-size:1rem; font-weight:700; color:var(--text-main);">Quick Actions & Analytics</h3>
              <a href="#progress-analytics" style="color:var(--primary); font-size:0.85rem; font-weight:600; text-decoration:none;">View Full Analytics →</a>
            </div>
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:14px;">
              <div class="card p-3" style="border-style:dashed; cursor:pointer;" onclick="openPlanModal()">
                <div style="color:var(--primary); font-size:1.3rem; margin-bottom:6px;"><i class="bi bi-calendar-plus"></i></div>
                <div style="font-weight:700; font-size:0.95rem;">Create Hifz Plan</div>
                <div style="font-size:0.75rem; color:var(--text-muted);">Set custom Surah targets</div>
              </div>
              <div class="card p-3" style="border-style:dashed; cursor:pointer;" onclick="window.location.hash='weak-verses'">
                <div style="color:var(--danger); font-size:1.3rem; margin-bottom:6px;"><i class="bi bi-lightning-charge"></i></div>
                <div style="font-weight:700; font-size:0.95rem;">Focus Weak Verses</div>
                <div style="font-size:0.75rem; color:var(--text-muted);">${weakCount} verses needing attention</div>
              </div>
              <div class="card p-3" style="border-style:dashed; cursor:pointer;" onclick="window.location.hash='profile'">
                <div style="color:var(--gold); font-size:1.3rem; margin-bottom:6px;"><i class="bi bi-award-fill"></i></div>
                <div style="font-weight:700; font-size:0.95rem;">Achievements</div>
                <div style="font-size:0.75rem; color:var(--text-muted);">Check unlocked milestones</div>
              </div>
            </div>
          </div>
        </div>
    ...
    container.innerHTML = `
      <div class="card p-4 text-center">
        <p class="text-danger">Failed to load dashboard data. Please try signing in again.</p>
...
function renderGuestHome(container) {
  container.innerHTML = `
    <div class="hero-card" style="margin-bottom: 24px;">
      <div>
        <div class="hero-greeting">Welcome to Quran Hifz & Muraja'ah</div>
        <div class="hero-sub">A systematic memorization and spaced-repetition revision platform for preserving the Holy Qur'an.</div>
        <div style="margin-top:20px; display:flex; gap:12px;">
          <button class="btn btn-gold" onclick="openAuthModal('login')">Sign In</button>
          <button class="btn btn-outline" style="color:#fff; border-color:#fff;" onclick="openAuthModal('register')">Create Account</button>
          <button class="btn btn-primary" onclick="demoSignIn()">Quick Demo Sign In</button>
        </div>
      </div>
    </div>

    <div class="card p-4">
      <h3 style="margin-bottom:12px; font-weight:700; color:var(--primary);">Features Included:</h3>
      <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:16px;">
        <div class="card p-3"><i class="bi bi-journal-text text-success"></i> <strong>Read & Search</strong> all 114 Surahs with authentic Arabic and translations.</div>
        <div class="card p-3"><i class="bi bi-repeat text-warning"></i> <strong>Spaced Repetition (SRS)</strong> scheduling that prioritizes weak verses automatically.</div>
        <div class="card p-3"><i class="bi bi-mic text-danger"></i> <strong>Recitation Recording</strong> using your browser microphone with server uploads.</div>
        <div class="card p-3"><i class="bi bi-fire text-primary"></i> <strong>Streak & Milestones</strong> tracking daily practice and unlocking achievements.</div>
      </div>
    </div>
  `;
}
    ...
    <div class="quran-header">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
        <h2 style="font-size:1.4rem; font-weight:800; color:var(--primary);">
          <i class="bi bi-book"></i> The Holy Qur'an
        </h2>
...
async function openSurahReader(surahNumber) {
  ...
  try {
      ...
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; flex-wrap:wrap; gap:10px;">
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
          ...
          <button class="btn btn-outline btn-sm" onclick="openRepeatModal()" title="Recitation Repeat Settings">
            <i class="bi bi-repeat"></i> <span id="reader-repeat-lbl">Repeat</span>
          </button>
          <button class="btn btn-gold btn-sm" onclick="startMemorizeForSurah(${surah.number})">
            <i class="bi bi-lightbulb-fill"></i> Memorize
          </button>
        </div>
      </div>
    ...
    container.innerHTML = `
      <div class="card p-4 text-center">
        <p class="text-danger font-weight-bold"><i class="bi bi-exclamation-triangle"></i> Failed to load Surah: ${e.message}</p>
    ...
    <div class="ayah-card" id="${cardId}" data-verse="${ayah.verse_key}">
      <div class="ayah-header">
        ...
        <div class="ayah-actions">
          <button class="icon-btn" title="Play Ayah Audio" onclick="playSingleAyahAudio('${ayah.verse_key}')">
            <i class="bi bi-play-fill"></i>
          </button>
          <button class="icon-btn" title="Bookmark Verse" onclick="promptBookmarkAyah(${ayah.id || 0})">
            <i class="bi bi-bookmark"></i>
          </button>
          <button class="icon-btn" title="Personal Note" onclick="promptNoteAyah(${ayah.id || 0})">
            <i class="bi bi-journal-plus"></i>
          </button>
          <button class="icon-btn" title="Practice this Ayah" onclick="quickPracticeAyah(${ayah.surah_number}, ${ayah.ayah_number})">
  ...
  container.innerHTML = `
    <div style="margin-bottom: 24px;">
      <h2 style="font-size:1.4rem; font-weight:800; color:var(--primary); margin-bottom:6px;">
        <i class="bi bi-mortarboard-fill"></i> Hifz Memorization Studio
      </h2>
    ...
    ${isGuest ? `
      <div class="alert alert-info p-3 mb-3" style="background:var(--primary-subtle); border:1px solid var(--border-gold); border-radius:var(--radius); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
        <div style="font-size:0.85rem; color:var(--text);">
          <i class="bi bi-info-circle-fill text-warning"></i> <strong>Guest Mode:</strong> You can read, listen, and practice freely! Sign in to save your Hifz streaks and SRS revision memory.
        </div>
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; flex-wrap:wrap; gap:10px;">
      ...
      <div style="display:flex; gap:8px;">
        <button class="btn btn-sm btn-primary" onclick="playCurrentSurahAudio()">
          <i class="bi bi-play-fill"></i> Play Verses
        </button>
          ...
          <div class="ayah-header">
            ...
            <div style="display:flex; gap:8px;">
              <button class="btn btn-sm btn-outline" onclick="markAyahMemorizedAction(${a.id || 0})">
                <i class="bi bi-check2-all text-success"></i> Mark Memorized
              </button>
              <button class="btn btn-sm btn-outline" onclick="markAyahDifficultAction(${a.id || 0})">
                <i class="bi bi-exclamation-circle text-warning"></i> Mark Difficult
              </button>
          ...
          ${mode !== 'test' ? `<div class="ayah-translation">${a.text_translation}</div>` : `
            <div style="text-align:center; padding:10px 0;">
              <button class="btn btn-sm btn-outline" onclick="toggleRevealArabic(${a.id})">
                <i class="bi bi-eye"></i> Tap to Check / Reveal Arabic Text
              </button>
            </div>
          `}
        </div>
  ...
  return `
    <div class="card p-4 mb-4 text-center" style="background:linear-gradient(135deg, var(--surface), var(--primary-subtle)); border:1px solid var(--border-gold);">
      <h3 style="font-size:1.1rem; font-weight:700; color:var(--primary); margin-bottom:8px;">
        <i class="bi bi-mic-fill"></i> Voice Recitation Studio
      </h3>
      ...
      <div style="display:flex; justify-content:center; align-items:center; gap:16px;">
        ...
        <button class="btn btn-gold" id="rec-upload-btn" style="display:none;" onclick="uploadCurrentRecording(${firstAyah.id})">
          <i class="bi bi-cloud-arrow-up"></i> Save Recording
        </button>
      </div>
...
async function toggleRecordingAction(ayahId) {
  ...
  if (AudioRecorder.isRecording()) {
    ...
    btn.innerHTML = `<i class="bi bi-record-circle"></i> Record Again`;
    ...
    try {
      ...
      btn.innerHTML = `<i class="bi bi-stop-circle-fill text-danger"></i> Stop Recording`;
...
async function uploadCurrentRecording(ayahId) {
  ...
  try {
    ...
    statusEl.innerHTML = `<span class="text-success"><i class="bi bi-check-circle-fill"></i> Recording saved successfully!</span>`;
  } catch (e) {
  ...
  box.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; border-bottom:1px solid var(--border); padding-bottom:12px;">
      <h3 style="font-size:1.18rem; font-weight:800; color:var(--primary); margin:0; display:flex; align-items:center; gap:8px;">
        <i class="bi bi-repeat"></i> Recitation Repeat Settings
      </h3>
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; gap:8px;">
      <button type="button" class="btn btn-outline btn-sm" onclick="quickCycleRepeatAction()">
        <i class="bi bi-arrow-repeat"></i> Quick Cycle Mode
      </button>
...
async function renderRevision(container) {
  if (!API.isAuthenticated()) {
    container.innerHTML = `
      <div class="hero-card" style="margin-bottom: 24px;">
        <div>
          <div class="hero-greeting"><i class="bi bi-arrow-repeat"></i> Muraja'ah & Spaced Repetition (SRS)</div>
          ...
          <div style="margin-top:20px; display:flex; gap:12px; flex-wrap:wrap;">
            <button class="btn btn-gold" onclick="demoSignIn()"><i class="bi bi-lightning-charge-fill"></i> ⚡ Quick Demo Sign In</button>
      ...
      <div class="card p-4">
        <h3 style="margin-bottom:12px; font-weight:700; color:var(--primary);"><i class="bi bi-shield-check"></i> How Muraja'ah Spaced Repetition Works:</h3>
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
      <div>
        <h2 style="font-size:1.4rem; font-weight:800; color:var(--primary); margin-bottom:4px;">
          <i class="bi bi-arrow-repeat"></i> Muraja'ah (Revision)
        </h2>
      ...
      <div>
        <button class="btn btn-gold" onclick="startMurajaahSession()">
          <i class="bi bi-play-circle-fill"></i> Start Interactive Session
        </button>
      </div>
    </div>
      ...
      <div class="revision-card">
        ...
        <div style="margin:16px 0;">
          <button class="btn btn-outline btn-sm" onclick="document.getElementById('revision-arabic-text').classList.toggle('hidden-mode')">
            <i class="bi bi-eye"></i> Hide / Reveal Arabic Text
          </button>
...
async function renderWeakVerses(container) {
  ...
  try {
    ...
    container.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
        <div>
          <h2 style="font-size:1.4rem; font-weight:800; color:var(--danger);">
            <i class="bi bi-shield-exclamation"></i> Weak Verses Priority Queue
          </h2>
          <p class="text-muted" style="font-size:0.9rem;">Verses with repeated mistakes or memorization strength below 70%.</p>
        </div>
        <div>
          <button class="btn btn-gold" onclick="startMurajaahSession()">
            <i class="bi bi-play-circle-fill"></i> Revise All Weak Verses
          </button>
        </div>
      </div>
      ...
      ${verses.length === 0 ? `
        <div class="card p-5 text-center">
          <div style="font-size:2.5rem; color:var(--primary); margin-bottom:10px;"><i class="bi bi-shield-check"></i></div>
...
async function renderProgressAnalytics(container) {
  ...
  try {
    ...
    container.innerHTML = `
      <div style="margin-bottom:24px;">
        <h2 style="font-size:1.4rem; font-weight:800; color:var(--primary); margin-bottom:4px;">
          <i class="bi bi-graph-up-arrow"></i> Progress & Memorization Analytics
        </h2>
      ...
      <div class="dashboard-grid mb-4">
        <div class="col-3">
          <div class="card">
            <div class="stat-widget">
              <div class="stat-icon-wrap stat-icon-emerald"><i class="bi bi-book"></i></div>
              <div>
        ...
        <div class="col-3">
          <div class="card">
            <div class="stat-widget">
              <div class="stat-icon-wrap stat-icon-gold"><i class="bi bi-award"></i></div>
              <div>
        ...
        <div class="col-3">
          <div class="card">
            <div class="stat-widget">
              <div class="stat-icon-wrap stat-icon-emerald"><i class="bi bi-bullseye"></i></div>
              <div>
        ...
        <div class="col-3">
          <div class="card">
            <div class="stat-widget">
              <div class="stat-icon-wrap stat-icon-gold"><i class="bi bi-fire"></i></div>
              <div>
...
async function renderProfile(container) {
  ...
  try {
    ...
    container.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px; flex-wrap:wrap; gap:12px;">
        <h2 style="font-size:1.4rem; font-weight:800; color:var(--primary);">
          <i class="bi bi-person-circle"></i> Student Profile & Settings
        </h2>
        <div style="display:flex; gap:8px;">
          ${user.role === 'admin' ? `<button class="btn btn-gold btn-sm" onclick="openAdminModal()"><i class="bi bi-shield-lock"></i> Admin Dashboard</button>` : ''}
        ...
        <div class="col-7">
          <div class="card card-gold-edge">
            <h3 style="font-size:1.1rem; font-weight:700; color:var(--gold); margin-bottom:16px;">
              <i class="bi bi-trophy-fill"></i> Achievements & Milestones
            </h3>
...
async function openAdminModal() {
  ...
  try {
    ...
    box.innerHTML = `
      <div class="modal-header">
        <h3 class="modal-title"><i class="bi bi-shield-lock-fill"></i> Admin System Oversight</h3>
        <button class="modal-close" onclick="closeModal()">&times;</button>
      </div>
