...
const API = (() => {
  ...
  return {
    ...
    // =============================================================
    // ISLAMIC LEARNING & WORSHIP PLATFORM API METHODS
    // =============================================================
    tajweed: {
      getCategories: () => request('/tajweed/categories'),
      getLessons: (categoryId) => request(`/tajweed/categories/${categoryId}/lessons`),
      getLesson: (id) => request(`/tajweed/lessons/${id}`),
      submitQuiz: (id, answers) => request(`/tajweed/lessons/${id}/quiz`, { method: 'POST', body: JSON.stringify({ answers }) }),
      getAyahRules: (surah, ayah) => request(`/tajweed/ayah-rules/${surah}/${ayah}`),
      getProgress: () => request('/tajweed/progress')
    },

    tafsir: {
      getSources: () => request('/tafsir/sources'),
      getAyahTafsir: (surah, ayah, sourceId = null) => request(`/tafsir/ayah/${surah}/${ayah}${sourceId ? `?source_id=${sourceId}` : ''}`),
      getSurahTafsir: (surah, sourceId = null, limit = 20, offset = 0) => request(`/tafsir/surah/${surah}?limit=${limit}&offset=${offset}${sourceId ? `&source_id=${sourceId}` : ''}`),
      search: (q, sourceId = null) => request(`/tafsir/search?q=${encodeURIComponent(q)}${sourceId ? `&source_id=${sourceId}` : ''}`)
    },

    tawhid: {
      getCategories: () => request('/tawhid/categories'),
      getLessons: (categoryId) => request(`/tawhid/categories/${categoryId}/lessons`),
      getLesson: (id) => request(`/tawhid/lessons/${id}`),
      submitQuiz: (id, answers) => request(`/tawhid/lessons/${id}/quiz`, { method: 'POST', body: JSON.stringify({ answers }) }),
      getProgress: () => request('/tawhid/progress')
    },

    prayer: {
      getCities: () => request('/prayer/cities'),
      getMethods: () => request('/prayer/methods'),
      getTimes: (params = {}) => {
        const query = new URLSearchParams(params).toString();
        return request(`/prayer/times${query ? `?${query}` : ''}`);
      },
      getMonthly: (params = {}) => {
        const query = new URLSearchParams(params).toString();
        return request(`/prayer/monthly${query ? `?${query}` : ''}`);
      },
      getSettings: () => request('/prayer/settings'),
      updateSettings: (data) => request('/prayer/settings', { method: 'PUT', body: JSON.stringify(data) })
    },

    qibla: {
      get: (lat, lng) => request(`/qibla?lat=${lat}&lng=${lng}`)
    },

    adhkar: {
      getCategories: () => request('/adhkar/categories'),
      getItems: (categoryId) => request(`/adhkar/categories/${categoryId}/items`),
      recordProgress: (dhikrId, count = 1) => request('/adhkar/progress', { method: 'POST', body: JSON.stringify({ dhikr_id: dhikrId, count }) })
    },

    duas: {
      getCategories: () => request('/duas/categories'),
      getItems: (categoryId) => request(`/duas/categories/${categoryId}/items`),
      search: (q) => request(`/duas/search?q=${encodeURIComponent(q)}`)
    },

    hadith: {
      getCollections: () => request('/hadith/collections'),
      getItems: (collectionId, limit = 20, offset = 0) => request(`/hadith/collections/${collectionId}/items?limit=${limit}&offset=${offset}`),
      getItem: (id) => request(`/hadith/${id}`),
      search: (q, collectionId = null) => request(`/hadith/search?q=${encodeURIComponent(q)}${collectionId ? `&collection_id=${collectionId}` : ''}`)
    },

    namesOfAllah: {
      getAll: () => request('/names-of-allah'),
      getByNumber: (num) => request(`/names-of-allah/${num}`)
    },

    hijri: {
      getToday: (adj = 0) => request(`/hijri/today?adj=${adj}`),
      getEvents: () => request('/hijri/events')
    },

    tasbih: {
      saveSession: (phrase, target, completed) => request('/tasbih/session', { method: 'POST', body: JSON.stringify({ phrase, target, completed }) }),
      getHistory: () => request('/tasbih/history')
    },

    unifiedBookmarks: {
      list: (type = null) => request(`/bookmarks/unified${type ? `?type=${type}` : ''}`),
      toggle: (data) => request('/bookmarks/unified/toggle', { method: 'POST', body: JSON.stringify(data) })
    },

    search: {
      global: (q, limit = 5) => request(`/search/global?q=${encodeURIComponent(q)}&limit=${limit}`)
    },
